<template>
	<view>
		<image src="http://image.jt.com/wechat/login.png" mode="widthFix" lazy-load></image>
		<view>
			<!-- 手机密码登录 -->
			<template v-if="!status">
				<view class="tel">
					<input placeholder="账号/手机号" v-model="userData" />
				</view>
				<view class="password">
	...
	methods: {
		...
		submit(){
			...
			if(this.status == false){
				// 账号登录
				uni.request({
					url:'http://localhost:8383/api/applogin/appLogin',
					data:{
						"keyword":this.userData,
		...
		messageLogin(){
			// 验证码登录
			console.log(this.checkNum,this.randomNumber)
			if(this.checkNum == this.randomNumber){
				uni.request({
					url:'http://localhost:8383/api/applogin/messageLogin',
					method:'POST',
					data:{
						phone:this.phone
					},
		...
		gotoAdd(){
			uni.navigateTo({
				url:'../companys/companys/companys'
			})
		}
